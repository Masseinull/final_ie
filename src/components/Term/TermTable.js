...
import TermTableHead from "./TermTableHead";
...
const TermTable = () => {
  ...
  const handleSelectAllClick = (e) => {
    if (e.target.checked) {
      const newSelected = general.TermData.map((n) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - general.TermData.length) : 0;
      ...
      <TableContainer>
        <TableStyle>
          ...
          <TermTableHead
            ...
            rowCount={general.TermData.length}
          />

          {/* Table Body */}
          <TableBody>
            {sortableArr(general.TermData, getComparator(order, orderBy))
              .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              .map((term, idx) => {
                const isItemSelected = selectedItems.indexOf(term.name) !== -1;
                  ...
                  >
                    <TableCell padding="checkbox">
                      <Checkbox
                        ...
                        onChange={(e) => handleClick(e, term.name)}
                      />
                    </TableCell>
