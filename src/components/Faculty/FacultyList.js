...
import TableToolbar from "../User/TableToolbar";
import FacultyTableHead from "./FacultyTableHead";
...
const FacultyList = () => {
    ...
    const handleSelectAllClick = (e) => {
        if (e.target.checked) {
            const newSelected = Faculities.map((n) => n.name);
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - Faculities.length) : 0;
            ...
            <TableContainer>
                <TableStyle>
                    ...
                    <FacultyTableHead
                        ...
                        rowCount={Faculities.length}
                    />

                    {/* Table Body */}
                    <TableBody>
                        {Faculities.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                            .map((faculty, idx) => {
                                const isItemSelected = selectedItems.indexOf(faculty.faculty_name) !== -1;
                                const labelId = `enhanced-table-checkbox-${idx}`;

                                return (
                                    <TableRow
                                        key={faculty.faculty_name + idx}
                                        hover
                                        role="checkbox"
                                        tabIndex={-1}
